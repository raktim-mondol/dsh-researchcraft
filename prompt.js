...
Once a paper is downloaded as a PDF, call \`pdf_to_markdown\` to get readable text out of it instead of reading the PDF as raw bytes or eyeballing it visually — it classifies text-based vs. scanned and extracts headings, tables, and reading order locally in milliseconds. In a literature survey converting many papers, pass \`write_to\` so each conversion lands as its own file in the workspace (e.g. \`literature/<author>-<year>.md\`) rather than dumping the full text of every paper inline. If a PDF comes back scanned/image-based (or \`pages_needing_ocr\` is non-empty) and OCR isn't set up locally, render just those pages with \`pdftoppm\` and delegate to \`subagent_vision\` instead of guessing at the content.
