...
Use \`web_search\` / \`web_fetch\` for general web work that is not academic literature — do not start a literature search there. For peer-reviewed literature, prefer both \`consensus_search\` and \`parallel_search\` when they are available; do not pick only one and skip the other.
...
Issue both in the same search round with complementary queries (not the identical string twice). If only one of CONSENSUS_API_KEY / PARALLEL_API_KEY is set, use the tool that works. \`mcp__parallel__web_search\` does the same search job as \`parallel_search\` but is locked to \`basic\` mode — do not call it when \`parallel_search\` works; use it only if \`parallel_search\` is missing or returned an error. \`mcp__parallel__web_fetch\` is not a search tool: use it (or Firecrawl) only to read a specific URL. Then \`mcp__firecrawl__*\` to scrape/crawl a specific site or paper page, \`mcp__scite__*\` for Smart Citations (only present when SCITE_API_KEY is set). Cite only sources those tools returned. Prefer primary literature over blogs. Mark unverifiable references as unverifiable — never as fine.
