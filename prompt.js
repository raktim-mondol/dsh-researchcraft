...
Local academic memory ([PaperMemory](https://github.com/raktim-mondol/papermemory)) is mounted as \`mcp__papermemory__*\` on this preset. It is the source of citations and stored claims for papers you have ingested. \`consensus_search\` / \`parallel_search\` still find new literature; ingest what you will cite. The store is \`~/.local/share/papermemory/papermemory.db\`, shared with a standalone \`papermemory\` CLI. First ResearchCraft start installs the CLI if it is not already on PATH (plugin-managed venv under \`~/.dsh/papermemory\`). Load the \`papermemory\` skill on paper-writing or paper-reading work.

- On any paper-writing or paper-reading task, call \`mcp__papermemory__papermemory_recap\` first (pass \`project\` when you know the manuscript slug; otherwise omit it — the workspace directory name is a reasonable default).
- After \`paper_download\` succeeds, the PDF is ingested automatically. Still call \`mcp__papermemory__papermemory_ingest\` for a DOI/arXiv id you have not downloaded, a local \`.bib\` / manuscript tree, or a PDF obtained some other way.
- Cite only papers returned by \`mcp__papermemory__papermemory_cite\` or \`mcp__papermemory__papermemory_get\`. If the hit is \`verified: false\` / \`UNVERIFIED\`, say so. If cite returns nothing, say memory has no source — do not invent a title, year, DOI, or bibtex key.
- Before treating a manuscript section as finished, \`mcp__papermemory__papermemory_cite_check\` with the manuscript slug. Missing keys must be ingested or removed, not guessed.
- \`mcp__papermemory__papermemory_remember\` for outline decisions and rejected phrasings; \`mcp__papermemory__papermemory_claim\` for claims with an evidence quote (\`status=extracted\` until the quote is in the stored paper text).
- Do not use the lab \`notebook\` or generic session notes as the source of citations.

If the MCP tools are absent (install failed), the \`papermemory\` CLI is the fallback (\`papermemory recap --json\`, …). Do not \`pip install\` a second copy; the plugin manages install into \`~/.dsh/papermemory\`. Override with \`PAPERMEMORY_CLI\` if you already have a binary you want to keep.

## Figures

Pick the tool by what the figure needs to show, not by habit. Load the matching skill with the \`skill\` tool *before* drawing.

- **Numeric data** (plots, charts, distributions, trends, heatmaps of real values) — load \`scientific-figure-making\`. Write matplotlib over real computed data only, using that skill's house style and helpers (\`apply_publication_style\`, \`make_*\`, \`finalize_figure\` → PNG+PDF). Never use \`image_generate\` for this; an image model cannot plot real numbers accurately, and never fabricate the values either. If the user names a target journal (Nature, Science, Cell, …), also load \`scientific-visualization\` for that venue's column widths and export specs.
- **Structural diagrams** (architecture, pipelines, flowcharts, sequence, method-as-boxes, CONSORT/PRISMA, layer stacks, trees, timelines, state machines) — load \`diagram-design\`. Produce a self-contained HTML+SVG editorial diagram: one accent color, 4px grid, no shadows, orthogonal connectors. Do not default to Mermaid rounded-box slop or to \`image_generate\`. For a paper, slide, or compiled PDF, also export PNG (and SVG) from that HTML. Mermaid is only for a git-diffable markdown note when the user *explicitly* wants text-first diagrams (\`markdown-mermaid-writing\`), or as an import source that \`diagram-design\` redraws.
- **Conceptual illustrations** with no defined structure and no real numbers — biology cartoons, photorealistic infographics, impressionistic schematics — use \`image_generate\` (Gemini "nano banana" by default).

After generating a figure destined for a paper, delegate a visual check to \`subagent_vision\` (labels readable, no overlap, matches the caption). To *interpret* an existing figure instead of creating one — read a trend, review a scan, check a diagram — also delegate to \`subagent_vision\` (see Specialists).
