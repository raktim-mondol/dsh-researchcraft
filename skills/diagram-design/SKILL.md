---
name: diagram-design
description: Create publication-quality architecture, flowchart, sequence, state machine, ER, timeline, swimlane, pipeline, method-schematic, CONSORT/PRISMA, layer-stack, tree, and 30 other editorial diagrams as self-contained HTML+SVG (export PNG/SVG for papers). Use for structural scientific figures — not for plots of real numbers (scientific-figure-making) and not for Mermaid rounded-box slop. Redraw draw.io, Mermaid, or Excalidraw sources. Use when the user asks for a diagram, schematic, architecture, flowchart, or pipeline figure.
license: MIT
metadata:
  version: "2.6"
  skill-source: https://github.com/cathrynlavery/diagram-design
---
...
This copy is the `cathrynlavery/diagram-design` skill, mapped onto this plugin.

- Load it with the `skill` tool. Then open **one** type reference from §3 — do not preload every file.
- **Do not deliver Mermaid as the figure.** Mermaid is only an import source (redraw it) or a git-diffable note when the user explicitly asks for text-first markdown (`markdown-mermaid-writing`). The deliverable is HTML+SVG.
- **Numeric plots of real data** (bars/lines/heatmaps of computed values) are `scientific-figure-making`, not this skill — even though this skill has bar/line/scatter types. Those types are for editorial diagrams, not for plotting a results table.
- **Skip §0 (website brand onboarding)** unless the user asks to match a site, venue, or saved profile. Scientific papers use the shipped default skin.
- This checkout **does not include the example gallery** (`example-*.html`). Start from `assets/template.html` (or `template-full.html` / `template-dark.html` / `template-motion.html`).
- Run Python through uv: `uv run python scripts/self_check.py <file>` from this skill directory (or with an absolute path). Same for the import extractors.
- **Paper / slide / LaTeX destination:** after the HTML is written, also export PNG (print scale 3) and SVG per `references/export.md`. Keep the HTML as the editable source. Install Playwright with uv if needed (`uv add playwright` in the workspace, then `uv run playwright install chromium`) — do not use bare `pip`.
- Log the figure path as a `notebook` artifact. For a paper figure, delegate a visual check to `subagent_vision` (labels readable, no overlap, accent on ≤2 nodes).

---

## 0. First-time setup — style guide gate

On ResearchCraft, skip this gate unless the user asked to match a brand, venue, or profile. Proceed with the shipped default skin.

**Before generating your first diagram in a new branded (non-paper) project, verify the style guide has been customized.**
...
- Plots of real numeric data (results tables, distributions, heatmaps of computed values) → `scientific-figure-making`.
